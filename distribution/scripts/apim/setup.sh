...
# Generate Keys
consumer_key=$($curl_command -H "Authorization: Bearer $subscribe_access_token" -H "Content-Type: application/json" -d "$(generate_keys_request)" "${base_https_url}/api/am/store/v0.11/applications/generate-keys?applicationId=$application_id" | jq -r '.consumerKey')
if [ ! -z $consumer_key ] && [ ! $consumer_key = "null" ]; then
    ...
    consumer_key=$($curl_command -H "Authorization: Bearer $subscribe_access_token" "${base_https_url}/api/am/store/v0.11/applications/$application_id" | jq -r '.keys[0] | .consumerKey')
    if [ ! -z $consumer_key ] && [ ! $consumer_key = "null" ]; then
...
create_api() {
    ...
    echo "Subscribing API to DefaultApplication"
